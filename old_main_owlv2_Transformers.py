...
for i in range(len(image_set)):
     ...
     current_id += result['detection_count'] # Увеличиваем для следующего изображения
                                        #для того, что бы id не был 1, для каждого Image
...
#вызов главной функции, для работы Qwen3
from Transformers_Qwen3_VL_4B_Instruct.Qwen_4_2 import main_qwen3
Qwen3_VL_MODEL_PATH = PROJECT_ROOT / 'Transformers_Qwen3_VL_4B_Instruct'

"""
# работал так себе с Qwen 3 vl 4b (transformers)
...
qwen3_resilt= main_qwen3(
     model_path= Qwen3_VL_MODEL_PATH, 
     #извлечем пути для 2-х обработанных файлов
     #image_path=[
          #owl_result[0]['visualization_path'], 
          #owl_result[1]['visualization_path']
     #],
     image_path=all_parts,
     text_input=text_search_objects, 
     )
